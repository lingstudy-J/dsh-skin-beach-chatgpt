...
- **Never touches DSH's own popovers** — the semantic tokens and stacking contexts behind the settings dialog, menus, toasts and code blocks stay official. The skin neither recolours their backgrounds nor puts `backdrop-filter` on a container that hosts a popover (that would pin the popover to the container's layer). The **one exception** is the sidebar list container: its 4px backdrop blur only quiets the wallpaper texture behind the text, and menus/popovers inside the list get a raised `z-index` as the safety net.
...
| Sidebar glass | 0–100% | **57%** | the sidebar should read as a floating glass menu on the wallpaper; hierarchy comes from shadow, not from a darker fill |
| Panel glass | 30–100% | **82%** | toolbars (bubbles and the composer have their own rows) |
| **Composer glass** | 80–100% | **82%** | the composer is the most solid layer on screen (the bottom anchor); **raise it if scrolling back bleeds text through** |
...
| Code blocks / terminal / tool cards | **not overridden** (deliberate) | follows the official theme: light background, dark text | dark background, light text |
