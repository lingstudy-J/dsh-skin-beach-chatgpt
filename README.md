...
- **一张连续的壁纸 + 两层可调玻璃**：壁纸只在 body 上画一次（`fixed` + `cover`），布局列本身透明，所以这张图从左栏到对话区是连续的；会话内容层与侧栏各自叠一层象牙白玻璃（默认 66% / 57%），**壁纸始终是主角**——没有任何一层是白板。壁纸之上还有一层 6% 的灰青薄雾，只用来压一压天空与海面的高光。
...
| **正文底衬** | 0 – 80% | **66%** | 会话内容层的象牙白玻璃，走**横向渐变**（左 72% → 中 66% → 右 52%）：左侧文字密、承托略强，右侧把海面和阳光让出来 |
...
1. **正文底衬**——默认已开在 66%；字还不够清楚就推到 80%（再高就会把壁纸遮得太多）；
