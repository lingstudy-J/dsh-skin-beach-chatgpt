...
- **一张连续的壁纸 + 两层可调玻璃**：壁纸只在 body 上画一次（`fixed` + `cover`），布局列本身透明，所以这张图从左栏到对话区是连续的；会话内容层与侧栏各自叠一层极轻的承托（默认 56% / 57%，正文那层用**灰青白**而不是暖白），**壁纸始终是主角**——没有任何一层是白板。壁纸之上还有一层 6% 的灰青薄雾，只用来压一压天空与海面的高光。
- **不碰 DSH 的弹层**：设置弹框、菜单、toast、代码块用到的语义 token 与层叠上下文全部保持官方行为——皮肤不覆盖它们的底色，也不在承载弹层的容器上加 `backdrop-filter`（那会把弹框压回容器所在的那一层）。**唯一例外**是侧栏列表容器：它的 4px 背景模糊只为压掉文字背后的壁纸纹理，并且已把列表内菜单/弹层的 `z-index` 抬到列表之上作兜底。
...
| 侧栏玻璃实度 | 0% – 100% | **57%** | 侧栏读作"漂浮在壁纸上的玻璃菜单"；层次靠阴影而不是加深底色，推到 0 就只剩壁纸 |
| 面板实度 | 30% – 100% | **82%** | 工具栏等的不透明度（气泡与输入框各有独立一档） |
| **输入框实度** | 80% – 100% | **82%** | 输入卡片是整屏最"实"的一层（底部锚点）；**往上滚历史时输入框文字被穿透就往上调** |
...
| **正文底衬** | 0 – 80% | **56%** | 会话内容层的**灰青白**承托（`235 241 239`，不是暖白），走**横向渐变**（左 61% → 中 56% → 右 32%）：变的是承托强弱，不是白膜浓度——右侧基本融入海景，让夕阳恢复颜色 |
...
1. **正文底衬**——默认已开在 56%；字还不够清楚就往上推（再高就会把壁纸遮得太多）；
...
```
海边背景（100% 展示，只压一层 6% 薄雾）
  ↓  顶部导航 40% · 侧栏 57%
  ↓  聊天正文 56%（灰青白，横向渐变：左 61% → 右 32%）
  ↓  用户消息 78% · 输入框 82%
  ↓  代码 / 终端：官方实体纸卡 + 青灰极细外框 + 轻悬浮阴影
```

所以终端内部那块近白的 output 是**刻意保留**的：它被 1px 青灰外框、10px 圆角、一层轻悬浮阴影和一栏略深的顶栏"装"进卡片里，读作主题里的**笔记纸**，而不是一块突兀的白。**没有**在 output 上加任何半透明蒙层、滤镜或不透明度。

**分层而不是统一**：皮肤不追求"每块都清晰、每处都有白底、所有组件同一个 rgba"。越需要阅读和交互的区域承托越强（输入框 82% > 用户消息 78% > 正文 66%），越属于环境与 chrome 的越透（侧栏 57% > 顶栏 40%），代码与终端则是官方实体纸卡。第一眼看到海，第二眼看到轻的青灰玻璃，第三眼才注意到功能组件。
...
| 小标题（h3–h6） | `--beach-ink-h3` | `#243C3E` | `#DCE6E4` |
| 占位文字 | `--beach-placeholder` | `#7D8E8F` | `#8FA0A0` |
