...
- **一张连续的壁纸 + 两层可调玻璃**：壁纸只在 body 上画一次（`fixed` + `cover`），布局列本身透明，所以这张图从左栏到对话区是连续的；会话内容层与侧栏各自叠一层象牙白玻璃（默认 76% / 68%），既保住通透感，又让长时间阅读的文字有稳定底衬。壁纸之上还有一层 6% 的灰青薄雾，只用来压一压天空与海面的高光。
...
| 侧栏玻璃实度 | 0% – 100% | **68%** | 侧栏玻璃，比会话内容层更透明一点（它不该比正文抢眼）；推到 0 就是完全透明、只剩壁纸 |
| 面板实度 | 30% – 100% | **82%** | 工具栏等的不透明度（气泡与输入框各有独立一档） |
| **输入框实度** | 80% – 100% | **88%** | 输入卡片浮在会话滚动区之上，透明度比普通卡片再高一点；**往上滚历史时输入框文字被穿透就往上调** |
...
| **正文底衬** | 0 – 80% | **76%** | 会话内容层的象牙白玻璃。**默认已开**——文字不再直接压在照片上；想让壁纸更透就往下调（建议不低于 72%） |
...
1. **正文底衬**——默认已开在 76%；字还不够清楚就推到 80%（再高就会把壁纸遮得太多）；
...
| 代码块底 / 字 | `--dsw-alias-markdown-code-block` / `--beach-code-block-ink` | `rgba(28,40,42,.92)` / `#EDF2EE` | `rgba(12,20,21,.93)` / `#EDF2EE` |
